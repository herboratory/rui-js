...
把 Rue.js 的三個 module folder 複製進去，再建立 `index.html`：
...
`data-i18n="home.title"` 的意思是：這個元素的文字由 `home.title` 這個翻譯 key 管理。
...
如果 Console 已經看到 JSON 成功載入，但畫面仍然空白，下一步應該看**第一個紅色 JavaScript error**，而不是繼續懷疑 fetch。
...
- **某個 Rue Notes helper not loaded**：照上面的 script 順序載入。
- **一次出現很多 ReferenceError**：先修 Console 最上面第一個 syntax error；前面的 script 一旦中斷，後面 functions 全部都不會建立。
- **`file://` CORS**：改用 local server。

## 9. Deploy 前人工驗收

至少測 EN／繁／简、JSON、每篇 Markdown、search、filter、所有連結、mobile，並確認 Console 沒有 uncaught error。
